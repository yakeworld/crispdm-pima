...
\begin{abstract}
Existing clinical ML guidelines universally warn against data leakage but provide no auditable implementation protocol. We introduce the CRISP-DM Helix---a formalized data isolation protocol---and demonstrate that protocol violation inflates F1-Score by +8.6\% while decreasing clinical Recall. The Pima Indians Diabetes Dataset (PIDD) is a cornerstone benchmark where current literature suffers from a severe ``high-metric, low-credibility'' paradox, with models claiming $>95\%$ accuracy universally failing in real-world deployment due to pervasive methodological flaws. To address this crisis, we propose the CRISP-DM Helix framework, a methodology that enforces absolute data isolation by encapsulating all preprocessing, imputation, and SMOTE applications strictly within cross-validation training folds. Our fully cross-validated ablation study provides quantitative measurement of metric inflation caused by data leakage, revealing a critical ``Recall Paradox''---severe leakage inflates F1-Score by +8.6\% (from 0.6759 to 0.7338) while paradoxically decreasing Recall for the diabetic minority class (from 0.7165 to 0.7080). Under strict isolation, an optimized soft-voting ensemble achieved an authentic F1-Score of 0.7541 and Recall of 0.7500. The CRISP-DM Helix is aligned with PROBAST and TRIPOD standards and provides a reusable template for any clinical ML study. Beyond the protocol itself, we propose a Dataset Baseline Registry: for each public benchmark, a verified performance ceiling under strict isolation serves as an automatic audit trigger---any study exceeding the ceiling should undergo mandatory methodological review. This framework establishes a verifiable performance baseline for the PIDD benchmark (F1=0.7541, Acc=0.7746) and operationalizes the detection of leakage-ridden studies across the literature.
\end{abstract}

\begin{keyword}
CRISP-DM \sep data leakage \sep diabetes prediction \sep process-driven credibility \sep machine learning reproducibility \sep benchmark baseline registry
\end{keyword}
...
\node[rectangle, rounded corners, minimum width=6cm, minimum height=1.2cm, text width=5.8cm, align=center, draw=black, fill=purple!10, line width=1.5pt] (core) at (5, 3.8) {\textbf{CRISP-DM Helix Core}\\\textit{Data Isolation Protocol}};

\draw[arrow] (c4.south) -- (core.north west);
\draw[arrow] (e4.south) -- (core.north east);

\node[box, below of=core, yshift=-0.5cm, minimum width=6cm, text width=5.8cm, align=center, fill=green!20] (output) {Output: Honest Performance Benchmark\\F1=0.7541, Recall=0.7500, Acc=0.7746};
\draw[arrow] (core.south) -- (output.north);

\node[box, below of=output, yshift=-0.5cm, minimum width=6cm, text width=5.8cm, align=center, fill=yellow!10] (ablation) {\textbf{Ablation Study:} 4 Leakage Levels\\No Leakage $\rightarrow$ Severe Leakage (+8.6\% F1)};
\draw[arrow] (output.south) -- (ablation.north);

\node[box, below of=ablation, yshift=-0.5cm, minimum width=6cm, text width=5.8cm, align=center, fill=red!10] (registry) {\textbf{Dataset Baseline Registry}\\Verified Performance Ceiling $\rightarrow$ Audit Trigger};
\draw[arrow, dashed] (ablation.south) -- (registry.north);

\end{tikzpicture}
\caption{The CRISP-DM Helix framework architecture, showing the dual-strand structure. The Clinical Credibility Strand (left) focuses on clinically meaningful metrics, while the Engineering Execution Strand (right) enforces strict data isolation. The Dataset Baseline Registry (bottom) operationalizes the performance ceiling as an automatic audit trigger for future work.}
\label{fig:helix}
\end{figure*}
...
\textbf{Second}, and most critically, catastrophic data leakage is rampant~\cite{Wen2024Leakage}. The vast majority of high-accuracy studies apply missing value imputation or SMOTE to the entire global dataset prior to cross-validation or train-test splitting~\cite{Wen2024Leakage}. This global preprocessing leaks the statistical distribution of the test set into the training phase, contaminating the model's evaluation.
...
While the current literature for Pima diabetes prediction is characterized by complex models claiming exceptionally high accuracies ($>95\%$), and existing reporting guidelines (TRIPOD, PROBAST, MI-CLAIM) all mandate the prevention of data leakage, two critical implementation gaps persist: (1) none provide a standardized, auditable protocol for achieving data isolation, and (2) none establish dataset-specific performance baselines that could serve as automatic detection mechanisms for leakage-ridden studies. The quantitative magnitude of leakage-induced metric distortion---the cost of protocol violation---remains unmeasured~\cite{Kapoor2024Leakage}.

\subsection{Occupying the Niche (Move 3 --- Our Contributions)}

To address this critical research gap, we propose the CRISP-DM Helix, a methodological framework that enforces strict data isolation by encapsulating all preprocessing, imputation, and resampling steps entirely within the training folds of the cross-validation loop. This paper establishes ``Process-Driven Credibility'' as a paradigm for clinical ML reproducibility. Our specific contributions are fivefold:
...
\begin{enumerate}[nosep]
    ...
    \item We establish \textbf{honest performance bounds} for the PIDD under strict Helix isolation (F1=0.7541, Acc=0.7746) through a benchmark of 34 models, and propose this baseline as a \textbf{Dataset Baseline Registry}---a community-maintained performance ceiling that serves as an automatic audit trigger.
...
To address the pervasive issue of statistical artifacts in existing literature, we introduce the \textbf{CRISP-DM Helix framework}~\cite{Kapoor2024Leakage}. This framework, built upon the PROBAST risk of bias assessment framework~\cite{Moons2019PROBAST}, conceptualizes the machine learning lifecycle as a double-helix structure consisting of two intertwined strands:
...
\subsection{Strict Data Preprocessing and Imputation}
...
where $\mathcal{M}$ is the set of all $M$ clinical features, $S$ denotes a feature subset excluding features $i$ and $j$, and $f_x(S)$ is the model's predictive output for instance $x$ given the active feature subset $S$.
...
\textbf{Backing:} The current PIDD literature is saturated with studies claiming near-perfect classification performance, including accuracies exceeding $95\%$, $99.81\%$, and even $100\%$~\cite{Mehta2024, Akbar2023, Chang2024, Deepalakshmi2025}. Our methodology directly challenges the validity of these claims. Our ablation study provides quantitative measurement of this leakage-induced inflation, demonstrating that the $+8.6\%$ F1-Score distortion is purely procedural rather than algorithmic~\cite{Wen2024Leakage}. As 92\% of recently surveyed PIDD studies violate fundamental data isolation principles~\cite{IllusionOfPerfection, Kapoor2024Leakage}, these extraordinarily high metrics are statistical artifacts generated by data leakage.

\subsection{Clinical Implications and the Baseline Registry}

\textbf{Qualifier:} In the context of diabetes screening, a diagnostic model that achieves $95\%$ global accuracy but misses $30\%$ of actual diabetic patients (false negatives) is clinically dangerous~\cite{Sali2025, Amri2025}. In medical settings, the consequences of a missed diagnosis far exceed those of a misdiagnosis, making high recall a clinical imperative~\cite{Amri2025, Sali2025}. Process integrity, enforced via the CRISP-DM Helix framework to isolate preprocessing within cross-validation folds, should serve as a standard for clinical ML reproducibility~\cite{Wen2024Leakage}.

Beyond the protocol itself, we propose that every public benchmark dataset in clinical ML should establish a \textbf{Dataset Baseline Registry}: a verified performance ceiling under a strict, leakage-free protocol. This registry would serve complementary functions.

First, it provides an \textbf{audit trigger}: any study reporting performance metrics substantially exceeding the baseline---for example, an F1-Score above 0.85 on the PIDD---automatically flags for rigorous methodological audit. Second, a registry of per-dataset baselines enables systematic detection of protocol violations across the literature, transforming ``everyone knows data leakage is bad'' from an abstract principle into an operational detection mechanism.

\textbf{The PIDD Baseline.} Our CRISP-DM Helix isolation protocol establishes a verified, fully transparent performance ceiling for the PIDD: an F1-Score of 0.7541 and Accuracy of 0.7746 under 34-model 10-fold stratified cross-validation. Any metric reported above this ceiling---particularly the ubiquitous claims of $>95\%$ or $>99\%$ accuracy in current literature---should be regarded as statistical artifacts of data leakage until proven otherwise via an auditable pipeline.

\textbf{Proposed Registry Architecture.} A community-maintained baseline registry could be organized as follows:

\begin{enumerate}[nosep]
    \item \textbf{Calibration}: For each dataset, the registry publishes a verified performance bound derived from a standardized, auditable pipeline.
    \item \textbf{Threshold}: A dataset-specific threshold calibrated so that metrics above it have a high probability ($p > 0.95$) of arising from methodological violations rather than algorithmic innovation.
    \item \textbf{Audit Protocol}: Any submission reporting metrics above the threshold must provide a completed PROBAST compliance checklist or equivalent methodological audit as a condition of peer review.
    \item \textbf{Community Governance}: The registry is version-controlled and maintained by a broad coalition of clinical ML researchers, analogous to PapersWithCode---but with methodological rigor as the primary filter.
\end{enumerate}

This registry architecture closes the loop that existing reporting guidelines leave open: TRIPOD, PROBAST, and MI-CLAIM all mandate ``prevent data leakage''~\cite{Collins2015TRIPOD, Moons2019PROBAST, Norgeot2020MI-CLAIM}, but none provide a dataset-specific threshold for detecting when leakage has \textit{actually} occurred. The baseline registry operationalizes this detection. Future work should extend this approach to other widely-used clinical ML benchmarks (e.g., MIMIC-III~\cite{Johnson2016MIMIC}, NHANES, Wisconsin Breast Cancer) and evaluate the sensitivity and specificity of different threshold metrics, guided by the reproducibility principles outlined in recent work~\cite{McDermott2021Reproducibility, Vollmer2020Machine}.
...
The CRISP-DM Helix framework addresses a critical gap in clinical ML research: while guidelines universally warn against data leakage, none provide a standardized implementation protocol or a dataset-specific baseline for detecting when violations have occurred. By offering a formal, auditable, and algorithmically complete protocol for data isolation---aligned with PROBAST, TRIPOD, and MI-CLAIM standards---the Helix framework transforms ``best practice'' from an aspirational principle into a verifiable pipeline component. Our ablation study quantifies the cost of protocol violation at $+8.6\%$ F1 inflation ($p < 0.05$), providing an empirical benchmark for evaluating pipeline integrity.

Beyond the protocol itself, we propose a \textbf{Dataset Baseline Registry} as a community mechanism for detecting leakage-ridden studies. The PIDD performance ceiling (F1=0.7541, Acc=0.7746) established here serves as the first entry in such a registry. Any study exceeding this ceiling should undergo mandatory methodological audit---closing the gap between ``leakage awareness'' and ``leakage detection.''

The CRISP-DM Helix is available as an open-source template that any clinical ML study can adopt. Future work should extend the framework to federated learning environments, real-time decision support systems, and multi-center external validation across diverse populations, as well as populate the Dataset Baseline Registry with calibrated baselines across additional clinical ML benchmarks.
